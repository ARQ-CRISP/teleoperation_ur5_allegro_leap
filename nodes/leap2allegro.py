...
if __name__ == "__main__":
    ...
    def do_calibrate(teleop_obj, pose_name, Finger1, Finger2, eps=1e-3):
        ...
        if dist <= eps or stop_calibration:
            ...
            ec.after(1000, lambda: do_calibrate(
                teleop_obj, pose_name, Finger1, Finger2, eps))

    ec.bind_action(
        'Escape', (None, allegro_teleop.toggle_tracking), 'Toggle Tracking mode')
    ec.bind_action(
        'space', (None, allegro_teleop.toggle_calibration_mode), 'Toggle Calibration mode')
    ec.bind_action('BackSpace', (None, exit_calibration_mode),
                   'Exit Calibration mode')
    ec.bind_action('F1', (lambda: calibrate_procedure(
        allegro_teleop, 'index', 'Index', 'Thumb', eps=1e-2), None), 'Calibrate Index')
    ec.bind_action('F2', (lambda: calibrate_procedure(
        allegro_teleop, 'middle', 'Middle', 'Thumb', eps=1e-2), None), 'Calibrate Middle')
    ec.bind_action('F3', (lambda: calibrate_procedure(
        allegro_teleop, 'ring', 'Ring', 'Thumb', eps=1.1e-2), None), 'Calibrate Ring')
    ec.show_keybinders()
